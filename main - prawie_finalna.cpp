...
double crc8(const vector<bitset<8>> &vec) {
	vector<int> bity;
	vector<int> div;
	const bitset<8> divisor = 137;

	for (int i = 0; i < vec.size(); i++) {
		for (int j = 7; j >= 0; j--) {
			bity.push_back(vec[i][j]);
		}
	}

	for (int i = divisor.size() - 1; i >= 0; i--) {
		div.push_back(divisor[i]);
	}

	for (int i = 0; i < divisor.size() - 1; i++)
		bity.push_back(0);

	//dla ka¿dego zestawu 4 bitów
	for (int i = 0; i <= bity.size() - div.size(); i++) {
		//przeskocz do pierwszej jedynki
		if (bity[i] == 1) {
			//podziel
	...
	for (int i = 0; i < div.size() - 1; i++) {
		suma += bity[bity.size() - i - 1] * pow(2, i);
	}
	return suma;
}
